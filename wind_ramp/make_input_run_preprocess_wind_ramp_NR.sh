...
for s in ${subdir[@]}; do
  ...
  for f in ${files[@]}; do
    ...
    path_arr=(${f//\// })
    echo ${path_arr[-1]::19}_wind_ramp_mask.nc >> ${out_fname}
  done
done

# Submit job to preprocess ceilings
cp ${script_dir}/wind_ramp/create_gen_wind_ramp_vx_mask_from_NR.sh .
sbatch create_gen_wind_ramp_vx_mask_from_NR.sh
