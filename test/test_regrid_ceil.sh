...
# Functional Test for the Regrid Data Plane Pipeline for Cloud Ceilings
#======================================================================
...
# Directory containing run_regrid_data_plane.sh
script_dir=/work2/noaa/wrfruc/murdzek/metplus_TEST
...
tmp_files=( in_files.txt out_files.txt 
            run_regrid_data_plane.sh metplus_${machine}.env regrid.out )
...
echo 'Running run_regrid_data_plane.sh...'
cp ${script_dir}/run_regrid_data_plane.sh .
cp ${script_dir}/metplus_${machine}.env .
cp ${script_dir}/py_${machine}.env .
bash run_regrid_data_plane.sh > regrid.out
err=$?
if [ ${err} -gt 0 ]; then
  echo "run_regrid_data_plane.sh raised error ${err}"
else
  echo "run_regrid_data_plane.sh ran without errors"
fi
